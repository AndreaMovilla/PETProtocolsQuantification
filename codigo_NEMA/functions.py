...
def thresholdseg(seg,image,voxeldim):
    ...
    while image[coord[0][0]][coord[1][0]][coord[2][0]]*0.7<image[coord[0][0]][coord[1][0]+testy_down][coord[2][0]]:
    ...
    for j in range(testz_down,testz_up):
        for i in range(testy_down,testy_up):
            testx_up=0
            testx_down=-1
            while image[coord[0][0]][coord[1][0]][coord[2][0]]*0.7<image[coord[0][0]+testx_up][coord[1][0]+i][coord[2][0]+j]:
                coord1.append((coord[0][0]+testx_up,coord[1][0]+i, coord[2][0]+j))
                testx_up=testx_up+1
            while image[coord[0][0]][coord[1][0]][coord[2][0]]*0.7<image[coord[0][0]+testx_down][coord[1][0]+i][coord[2][0]+j]:
